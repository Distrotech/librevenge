...
static guint16 multinationalMap[] = {
...
{
	guint16 extendedCharacter = (guint16)' '; /* whitespace in undefined case */

	switch (m_characterSet) 
	  {
	   case WP6_MULTINATIONAL_CHARACTER_SET:
		  if (m_character < WP6_NUM_MULTINATIONAL_CHARACTERS)
		    extendedCharacter = multinationalMap[m_character];
		  break;
	   case WP6_PHONETIC_SYMBOL_CHARACTER_SET:
		  if (m_character < WP6_NUM_PHONETIC_CHARACTERS)
		    extendedCharacter = phoneticMap[m_character];
		  break;
	   case WP6_TYPOGRAPHIC_SYMBOL_CHARACTER_SET:
		  if (m_character < WP6_NUM_TYPOGRAPHIC_CHARACTERS)
		    extendedCharacter = typographicMap[m_character];
		  break;
	   case WP6_ICONIC_SYMBOL_CHARACTER_SET:
		  if (m_character < WP6_NUM_ICONIC_CHARACTERS)
		    extendedCharacter = iconicMap[m_character];
		  break;
	   case WP6_MATH_SCIENTIFIC_CHARACTER_SET:
		  if (m_character < WP6_NUM_MATH_SCIENTIFIC_CHARACTERS)
		    extendedCharacter = mathMap[m_character];       
		  break;
	   case WP6_MATH_SCIENTIFIC_EXTENDED_CHARACTER_SET:
		  if (m_character < WP6_NUM_MATH_SCIENTIFIC_EXTENDED_CHARACTERS)
		    extendedCharacter = mathextMap[m_character];       
		  break;
	   case WP6_GREEK_CHARACTER_SET:
		  if (m_character < WP6_NUM_GREEK_CHARACTERS)
		    extendedCharacter = greekMap[m_character];       
		  break;
	   case WP6_HEBREW_CHARACTER_SET:
		  if (m_character < WP6_NUM_HEBREW_CHARACTERS)
		    extendedCharacter = hebrewMap[m_character];       
		  break;
	   case WP6_CYRILLIC_CHARACTER_SET:
		  if (m_character < WP6_NUM_CYRILLIC_CHARACTERS)
		    extendedCharacter = cyrillicMap[m_character];       
		  break;
	   case WP6_JAPANESE_CHARACTER_SET:
		  if (m_character < WP6_NUM_JAPANESE_CHARACTERS)
		    extendedCharacter = japaneseMap[m_character];
		  break;
	  }

	llListener->insertCharacter(extendedCharacter);
}
