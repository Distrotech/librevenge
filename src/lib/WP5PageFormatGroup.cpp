...
WP5PageFormatGroup::WP5PageFormatGroup(RVNGInputStream *input, RVNGEncryption *encryption) :
...
void WP5PageFormatGroup::_readContents(RVNGInputStream *input, RVNGEncryption *encryption)
{
	...
	case WP5_TOP_PAGE_FORMAT_GROUP_LEFT_RIGHT_MARGIN_SET:
		...
		RVNG_DEBUG_MSG(("WordPerfect: Page format group left/right margin set (left margin: %i, right margin: %i)\n", m_leftMargin, m_rightMargin));
		break;
	case WP5_TOP_PAGE_FORMAT_GROUP_SPACING_SET:
		...
		input->seek(2, RVNG_SEEK_CUR);
		{
			...
			RVNG_DEBUG_MSG(("WordPerfect: Page format group line spacing - integer part: %i fractional part: %f (original value: %i)\n",
	...
	case WP5_TOP_PAGE_FORMAT_GROUP_TAB_SET:
		...
		m_tabStops.reserve(40);
		{
			...
			{
				m_tabStops.push_back(RVNGTabStop());
				m_tabStops[i].m_position = (double)((double)tmpTabPosition/(double)RVNG_NUM_WPUS_PER_INCH);
			}
			...
			if ((getSize() > 4) && (getSize() - 4 == 0x00D0))
			{
				...
				{
					for (std::vector<RVNGTabStop>::iterator iter = m_tabStops.begin(); iter != m_tabStops.end(); ++iter)
						iter->m_position -= (double)((double)m_marginOffset/(double)RVNG_NUM_WPUS_PER_INCH);
				}
			}
			else
				m_marginOffset = 0xFFFF;
	...
	case WP5_TOP_PAGE_FORMAT_GROUP_TOP_BOTTOM_MARGIN_SET:
		...
		RVNG_DEBUG_MSG(("WordPerfect: Page format group top/bottom margin set (top margin: %i, bottom margin: %i)\n", m_topMargin, m_bottomMargin));
		break;
	case WP5_TOP_PAGE_FORMAT_GROUP_JUSTIFICATION:
		...
		RVNG_DEBUG_MSG(("WordPerfect: Page format group justification (0x%2x)\n", m_justification));
		break;
...
void WP5PageFormatGroup::parse(WP5Listener *listener)
{
	...
	{
	case WP5_TOP_PAGE_FORMAT_GROUP_LEFT_RIGHT_MARGIN_SET:
		listener->marginChange(RVNG_LEFT, m_leftMargin);
		listener->marginChange(RVNG_RIGHT, m_rightMargin);
		break;
	case WP5_TOP_PAGE_FORMAT_GROUP_SPACING_SET:
		...
		break;
	case WP5_TOP_PAGE_FORMAT_GROUP_TAB_SET:
		RVNG_DEBUG_MSG(("WordPerfect: parsing a tab set\n"));
		listener->setTabs(m_tabStops, m_marginOffset);
		break;
	case WP5_TOP_PAGE_FORMAT_GROUP_TOP_BOTTOM_MARGIN_SET:
		listener->pageMarginChange(RVNG_TOP, m_topMargin);
		listener->pageMarginChange(RVNG_BOTTOM, m_bottomMargin);
		break;
	case WP5_TOP_PAGE_FORMAT_GROUP_JUSTIFICATION:
