...
RVNGHeader::RVNGHeader(RVNGInputStream * /* input */, RVNGEncryption * /* encryption */, uint32_t documentOffset, uint8_t productType,
...
RVNGHeader::~RVNGHeader()
{
}

RVNGHeader *RVNGHeader::constructHeader(RVNGInputStream *input, RVNGEncryption *encryption)
{
	RVNG_DEBUG_MSG(("RVNGHeader::constructHeader()\n"));

	char fileMagic[4] = { 0, 0, 0, 0 };
	/* check the magic */
	input->seek(RVNG_HEADER_MAGIC_OFFSET, RVNG_SEEK_SET);
	for (int i=0; i<3 && !input->atEOS(); i++)
		fileMagic[i] = (char)readU8(input, encryption);

	if ( strcmp(fileMagic, "WPC") )
	{
		RVNG_DEBUG_MSG(("WordPerfect: File magic is not equal to \"WPC\"!\n"));
		return 0;
	}

	/* get the document pointer */
	input->seek(RVNG_HEADER_DOCUMENT_POINTER_OFFSET, RVNG_SEEK_SET);
	uint32_t documentOffset = readU32(input, encryption);

	/* get information on product types, file types, versions */
	input->seek(RVNG_HEADER_PRODUCT_TYPE_OFFSET, RVNG_SEEK_SET);
	...
	input->seek(RVNG_HEADER_ENCRYPTION_OFFSET, RVNG_SEEK_SET);
	uint16_t documentEncryption = readU16(input, encryption);

	RVNG_DEBUG_MSG(("WordPerfect: Document Offset = 0x%x \n",documentOffset));
	RVNG_DEBUG_MSG(("WordPerfect: Product Type: %i File Type: %i Major Version: %i Minor Version: %i\n",
	               productType, fileType,
	               majorVersion, minorVersion));
	RVNG_DEBUG_MSG(("WordPerfect: Document Encryption = 0x%x \n",(int)documentEncryption));


	switch (fileType)
	{
	case 0x0a:
		RVNG_DEBUG_MSG(("WordPerfect: Supported file type: \"WordPerfect Document\"\n"));
		switch (majorVersion)
		{
		case 0x00: // WP5
		...
		default:
			...
			RVNG_DEBUG_MSG(("WordPerfect: Unsupported major number: %d\n", majorVersion));
			return 0;
		}
	case 0x2c:
		RVNG_DEBUG_MSG(("WordPerfect: Supported file type: \"MAC WP 2.0 document\"\n"));
		switch (majorVersion)
		{
	...
	default:
		RVNG_DEBUG_MSG(("WordPerfect: Unsupported file type: %d\n", fileType));
		return 0;
	}
}
