...
void WPXContentListener::_openSpan()
{
	...
	if (m_ps->m_fontName)
	{
		if (*(m_ps->m_fontName) == "Symbol")
			propList.insert("style:font-name", "WGL4");
		else
...
uint16_t WPXContentListener::_mapSymbolFont(uint16_t character)
{
	static uint16_t _symbolFontMap1 [] =
	{
		0x0020, 0x0021, 0x2200, 0x0023, 0x2203, 0x0025, 0x0026, 0x220D, // 0x21 ..
	...
	static uint16_t _symbolFontMap2 [] =
	{
	...
	if (character >= 0x0020 && character < 0x7F)
		return _symbolFontMap1[character - 0x20];
	if (character >= 0x00A0 && character < 0xFF)
