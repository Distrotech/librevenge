...
void WP6HLContentListener::spacingBeforeParagraphChange(const gint16 spacing)
{
	if (!isUndoOn())
	{
...
void WP6HLContentListener::_openListElement()
{
	m_listenerImpl->openListElement(m_parseState->m_paragraphJustification,
					...
					m_parseState->m_paragraphLineSpacing, m_ps->m_paragraphSpacingBefore, m_ps->m_paragraphSpacingAfter);
